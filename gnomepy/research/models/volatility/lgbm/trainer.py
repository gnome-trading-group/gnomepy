...
from gnomepy.research.models.volatility.lgbm.features import VOL_FEATURE_NAMES, extract_vol_features_bulk
...
class LGBMVolatilityTrainer:
    ...
    def train(
        ...
        if register:
            ...
            registered_model = LGBMVolatilityModel(horizon=self.horizon)
            registered_model.model = model
            registered_model.listing_id = self.listing_id
            metadata = self._build_metadata(params, model, fold_results)
            version = registry.register(registered_model, metadata)
    ...
    def _build_metadata(
        self, params: dict, model: object, fold_results: list[FoldResult]
    ) -> dict:
